...
def parse_delivery_policy(payload: Mapping[str, object]) -> DeliveryPolicy:
    ...
    for field in ("router_revision", "evidence_set_sha256", "evidence_gate_sha256"):
...
def _candidate_reasons(candidate: DeliveryCandidate) -> list[str]:
    reasons = []
    if candidate.transition_id <= 0 or candidate.candidate_id <= 0 or candidate.rank_run_id <= 0:
...
def evaluate_delivery_readiness(
    ...
    *,
    ...
    if (
        ...
        for reason in ("DATA_STALE", "DATA_FROM_FUTURE", "EVIDENCE_FROM_FUTURE")
    ):
    ...
    return DeliveryReadinessDecision(
        ...
        idempotency_key=f"postmarket-readiness-v1:{_sha256_json(identity)}",
        policy_sha256=policy.sha256,
