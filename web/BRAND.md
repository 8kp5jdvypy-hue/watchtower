...
**`index.html`'s favicon** is a hand-encoded inline
`data:image/svg+xml` URI. Static HTML can't `import` a JS module at
request time, so this is a genuine, permanent duplicate of
`FALCON_PATHS` — not a bug, just the nature of favicons. Left as-is in
this pass (changing it wasn't necessary to unblock the future swap, and
"the website should look essentially unchanged" was an explicit
constraint). Once `public/brand/perch-mark.svg` exists, replace this
line with `<link rel="icon" href="/brand/perch-mark.svg" />` — a real
file reference, no more inline duplication.
...
2. **`index.html`** — swap the inline favicon `data:` URI for
   `<link rel="icon" href="/brand/perch-mark.svg" />`.
