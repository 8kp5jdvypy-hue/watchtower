...
(async () => {
  ...
  let device;
  await run('registerDevice -> Device (M2)', async () => { device = await client.registerDevice({ platform: 'ios', apnsEnvironment: 'sandbox', token: 'ab'.repeat(32), installationId: '9d2c1b9a-1b2c-4d3e-8f4a-5b6c7d8e9f00', appVersion: '1', locale: 'en', timeZone: 'America/New_York', preferences: { watchlistPriority: true, radarDiscoveries: false, corrections: true, operational: true, sessions: ['regular'], paused: false } }, '2f1e0d9c-8b7a-4655-9443-322110ffeedd'); return `id=${device.id.slice(0, 8)} active=${device.active}`; });
  await run('unregisterDevice -> 204', async () => { await client.unregisterDevice(device.id); return 'ok'; });
